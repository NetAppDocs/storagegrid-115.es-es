...
Consulte «'Cómo los nodos de grid detectan el nodo de administración principal».
...
Para obtener más información sobre la clonación de direcciones MAC, consulte las consideraciones y recomendaciones para la clonación de direcciones MAC.
...
Sólo SE requiere BLOCK_DEVICE_RANGEDB_00; el resto es opcional. El dispositivo de bloque especificado para BLOCK_DEVICE_RANGEDB_00 debe tener al menos 4 TB; los demás pueden ser más pequeños.

*Nota*: No deje huecos. Si especifica BLOCK_DEVICE_RANGEDB_05, también debe especificar BLOCK_DEVICE_RANGEDB_04.
...
*Nota*: El valor de la RAM afecta al espacio reservado real de metadatos de un nodo. Consulte las instrucciones para administrar StorageGRID para obtener una descripción de lo que es el espacio reservado de metadatos.
...
El formato utilizado es: `<network type>/<protocol:>/<remapped port >/<default port used by grid node>`, donde tipo de red es grid, administrador o cliente y protocolo es tcp o udp.
